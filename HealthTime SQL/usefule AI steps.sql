change the query, find the record after a change of HIRE_DT
if there are seveval HIRE_DT, find the record after a change of HIRE_DT with the most recent HIRE_DT

create a new QA query for emplid=10361877, trace the logic how to get effdt, hire_dt, and list all filtered records

create satge SP_UKG_EMPL_DATA_Update_imgr-Step3, add the logic in the comments to explain the SP

rename to satge SP_UKG_EMPL_DATA_CleanUp-Step1, add the logic in the comments to explain the SP


40657171

QA reports_to 40657171 should have [reports to manager] 10370316
based on the query below, [reports to manager] is '', I need to know how it was generated from the SP, QA one record reports_to='40657171'
SELECT top 1
reports_to, [reports to manager]
FROM [dbo].[UKG_EMPLOYEE_DATA] A
WHERE 
reports_to='40657171'
