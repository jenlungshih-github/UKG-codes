...
/****** Object:  StoredProcedure [stage].[SP_UKG_EMPL_STATUS_LOOKUP_BUILD_RETRO_ONLY]    Script Date: 10/28/2025 12:19:30 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [stage].[SP_UKG_EMPL_STATUS_LOOKUP_BUILD_RETRO_ONLY]
AS
BEGIN
    -- Drop table and index if they exist
    IF OBJECT_ID('[stage].[UKG_EMPL_STATUS_LOOKUP_RETRO_ONLY]') IS NOT NULL
        DROP TABLE [stage].[UKG_EMPL_STATUS_LOOKUP_RETRO_ONLY];
    ...
    WITH
        ...
        (
            ...
            WHERE EFFDT <= GETDATE() -- Consider records up to the current date
                ...
                AND JOB_INDICATOR='P'
                AND EFFDT >= HIRE_DT
                AND emplid IN ('10664090', '10556054')
