...
/****** Object:  StoredProcedure [stage].[SP_UKG_EMPLOYEE_DATA_DAILY_INCREMENTAL_HISTORY_INSERT]    Script Date: 2/18/2026 9:12:42 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    DECLARE @today_TXT VARCHAR(10) = CONVERT(VARCHAR(10), @today, 23);
    ...
    BEGIN TRY
        ...
        [termination_dt], [action], [action_dt], [hash_value], [NOTE], [snapshot_date_TXT], [snapshot_date]
        )
    SELECT
        ...
        HASHBYTES('md5', CONCAT(src.EMPLID, src.DEPTID, src.VC_CODE, src.hr_status, src.empl_Status, src.termination_dt, src.action, src.action_dt)) AS hash_value,
        'I' AS NOTE,
        @today_TXT AS snapshot_date_TXT,
...
--            AND check_today.snapshot_date_TXT = @today_TXT
