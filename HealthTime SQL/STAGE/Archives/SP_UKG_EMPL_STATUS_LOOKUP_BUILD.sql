...
/****** Object:  StoredProcedure [stage].[SP_UKG_EMPL_STATUS_LOOKUP_BUILD]    Script Date: 6/10/2025 10:18:02 AM ******/
SET ANSI_NULLS ON
GO
...
ActualChangePoints AS (
    ...
    SELECT
        ...
        EFFSEQ,
		EMPL_RCD,
        previous_EMPL_STATUS, -- Keep for clarity if needed, though not strictly used by rn_of_change
...
FROM (
    -- Dataset 1: Oldest record for each employee (lor)
    SELECT
        sc.emplid,
        sc.EMPL_STATUS,
        sc.EFFDT,
...
--	and emplid=10000644
    ...
    -- Dataset 2: Latest change record for each employee (lcr)
    SELECT
        acp.emplid,
        acp.EMPL_STATUS,
        acp.EFFDT,
