...
(
            ...
            BEGIN
                ...
                PRINT '    - WARNING: Position ' + @POSITION_REPORTS_TO + ' missing in PS_JOB';
            END
            ELSE
            BEGIN
                SET @NOTE = 'found in PS_JOB - EMPLID: ' + ISNULL(@PS_JOB_EMPLID, 'NULL') + ', HR_STATUS: ' + ISNULL(@PS_JOB_HR_STATUS, 'NULL');
                PRINT '    - OK: Position ' + @POSITION_REPORTS_TO + ' found in PS_JOB - EMPLID: ' + ISNULL(@PS_JOB_EMPLID, 'NULL') + ', HR_STATUS: ' + ISNULL(@PS_JOB_HR_STATUS, 'NULL');
            END
        ...
        SET @UpdateSQL = N'
        UPDATE [stage].[UKG_PositionReportsAnalysis_TEMP]
        SET NOTE = @NOTE,
    ...
    -- Create level by level tracking table with LEVEL UP column
    IF OBJECT_ID('[stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL]', 'U') IS NOT NULL DROP TABLE [stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL];

    SELECT *,
        1 as [LEVEL UP]
    INTO [HealthTime].[stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL]
