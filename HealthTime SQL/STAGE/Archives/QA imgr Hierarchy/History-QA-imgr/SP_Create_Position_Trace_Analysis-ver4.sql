...
BEGIN
    ...
    SELECT DISTINCT
        ...
        COALESCE(A.MANAGER_POSITION_NBR_L1, t1.MANAGER_POSITION_NBR) as MANAGER_POSITION_NBR,
        t1.POSN_LEVEL,
        t1.To_Trace_Up_1,
        --A.MANAGER_POSITION_NBR_L1,
        COALESCE(A.MANAGER_POSITION_NBR_L1, t1.MANAGER_POSITION_NBR) as MANAGER_POSITION_NBR_L1,
        COALESCE(A.[PS_JOB_EMPLID], X.[PS_JOB_EMPLID]) as MANAGER_EMPLID,
        COALESCE(A.[PS_JOB_HR_STATUS], X.[PS_JOB_HR_STATUS]) as MANAGER_HR_STATUS,
        COALESCE(A.[POSN_STATUS], X.[POSN_STATUS]) as MANAGER_POSN_STATUS,
        COALESCE(L.POSN_LEVEL, L2.POSN_LEVEL) as MANAGER_POSN_LEVEL,
        CASE 
            WHEN COALESCE(A.[PS_JOB_HR_STATUS], X.[PS_JOB_HR_STATUS]) = 'A' THEN 'no'
            ELSE 'yes'
    ...
    FROM #temp1 t1
        LEFT JOIN (
            SELECT
            [POSITION_REPORTS_TO] AS MANAGER_POSITION_NBR_L1,
            [PS_JOB_EMPLID],
            [PS_JOB_HR_STATUS],
            [POSN_STATUS]
        ...
        ) A ON t1.MANAGER_POSITION_NBR = A.MANAGER_POSITION_NBR_L1
        LEFT JOIN [stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL] X
        ON t1.MANAGER_POSITION_NBR = X.[Inactive_EMPLID_POSITION_NBR]
            AND A.MANAGER_POSITION_NBR_L1 IS NULL
            AND X.NOTE <> 'missing in PS_JOB'
        LEFT JOIN [stage].[UKG_EMPL_HIERARCHY_POSN_LOOKUP] L
        ON A.MANAGER_POSITION_NBR_L1 = L.POSITION_NBR
        LEFT JOIN [stage].[UKG_EMPL_HIERARCHY_POSN_LOOKUP] L2
        ON X.[POSITION_REPORTS_TO] = L2.POSITION_NBR
