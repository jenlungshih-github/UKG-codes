...
-- exec [stage].[SP_UKG_Position_Reports_Analysis_Level_UP] @InputLevel = 4  -- For Level 4 -> Level 5
/***************************************
* Created By: Jim Shih	
...
BEGIN
    ...
    ELSE 
    BEGIN
        SET @SourceTable = '[HealthTime].[stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL]';
        PRINT 'Processing Level ' + CAST(@InputLevel AS VARCHAR(10)) + ' -> Level ' + CAST(@OutputLevel AS VARCHAR(10)) + ': Source = UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL where [LEVEL UP] = ' + CAST(@InputLevel AS VARCHAR(10));
    END
    ...
    BEGIN
        ...
        WHERE [LEVEL UP] = @InputLevel
            AND [PS_JOB_EMPLID] IS NOT NULL;
    END
    ...
    FROM (
        SELECT DISTINCT
            CASE WHEN @InputLevel = 1 THEN t1.[PS_JOB_EMPLID] ELSE level_data.[PS_JOB_EMPLID] END as Inactive_EMPLID,
            CASE WHEN @InputLevel = 1 THEN t1.[POSITION_REPORTS_TO] ELSE level_data.[POSITION_REPORTS_TO] END as Position_Reports_To
        FROM [HealthTime].[stage].[UKG_PositionReportsAnalysis_TEMP] t1
            FULL OUTER JOIN [HealthTime].[stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL] level_data
            ON 1=0
        -- Never join, just for structure
        WHERE (@InputLevel = 1 AND t1.[PS_JOB_EMPLID] IS NOT NULL)
            OR (@InputLevel > 1 AND level_data.[LEVEL UP] = @InputLevel AND level_data.[PS_JOB_EMPLID] IS NOT NULL)
    ) imgr
    ...
    -- Debug: Check for duplicate POSITION_REPORTS_TO mappings for position 40689748
    IF @InputLevel > 1
    BEGIN
        PRINT 'Debug: Checking CURRENT_EMPL_DATA for position 40689748';
        SELECT
            'CURRENT_EMPL_DATA Debug' as DebugStep,
            emplid,
            NAME,
            POSITION_NBR,
            MANAGER_EMPLID,
        ...
        WHERE POSITION_NBR = '40689748';

        PRINT 'Debug: Checking source temp table for position 40689748';
        SELECT
            'Level BY Level Debug' as DebugStep,
            Inactive_EMPLID,
            POSITION_REPORTS_TO,
            PS_JOB_EMPLID,
            [LEVEL UP]
        FROM [HealthTime].[stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL]
        WHERE POSITION_REPORTS_TO = '40689748'
            AND [LEVEL UP] = @InputLevel;
    END
    ...
    -- For Level 2->3: Use UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL where [LEVEL UP] = @InputLevel
    ...
    WITH
        ...
        (
            SELECT
                ...
                MANAGER_NAME,
                POSITION_REPORTS_TO,
                ROW_NUMBER() OVER (PARTITION BY POSITION_NBR ORDER BY emplid) as RN
            FROM health_ods.health_ods.[RPT].[CURRENT_EMPL_DATA]
        )
    SELECT DISTINCT
        CASE 
            WHEN @InputLevel = 1 THEN temp1.[PS_JOB_EMPLID]
            ELSE level_data.[PS_JOB_EMPLID]  -- Use PS_JOB_EMPLID from tracking table
        END as Inactive_EMPLID,
        CASE 
            WHEN @InputLevel = 1 THEN temp1.[POSITION_REPORTS_TO]  -- For Level 1->2: Use the position they report to
            ELSE level_data.[POSITION_REPORTS_TO]  -- For Level 2+: Use POSITION_REPORTS_TO from tracking table
        END as Inactive_EMPLID_POSITION_NBR,
        CASE 
            WHEN @InputLevel = 1 THEN empl1.MANAGER_EMPLID
            ELSE empl2.MANAGER_EMPLID
        END as MANAGER_EMPLID,
        CASE 
            WHEN @InputLevel = 1 THEN empl1.MANAGER_NAME
            ELSE empl2.MANAGER_NAME
        END as MANAGER_NAME,
        CASE 
    ...
    FROM [HealthTime].[stage].[UKG_PositionReportsAnalysis_TEMP] temp1
        FULL OUTER JOIN [HealthTime].[stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL] level_data
        ON 1=0 -- Never join, structure only
        LEFT JOIN CurrentEmplData empl1 -- For Level 1: Join by position number with RN=1
        ON empl1.POSITION_NBR = temp1.[POSITION_REPORTS_TO]
            AND empl1.RN = 1
        LEFT JOIN CurrentEmplData empl2 -- For Level 2+: Join by position number with RN=1
        ON empl2.POSITION_NBR = level_data.[POSITION_REPORTS_TO]
            AND empl2.RN = 1
        LEFT JOIN PositionData pd -- For Level 2+: Get position data for POSITION_REPORTS_TO
        ON pd.POSITION_NBR = empl2.[POSITION_REPORTS_TO]
            AND pd.RN = 1
        LEFT JOIN PositionData pd1 -- For Level 1: Get position data for POSITION_REPORTS_TO
        ON pd1.POSITION_NBR = empl1.[POSITION_REPORTS_TO]
            AND pd1.RN = 1
    WHERE (
        (@InputLevel = 1 AND temp1.[PS_JOB_EMPLID] IS NOT NULL AND temp1.[POSITION_REPORTS_TO] IS NOT NULL) OR
        (@InputLevel > 1 AND level_data.[LEVEL UP] = @InputLevel AND level_data.[PS_JOB_EMPLID] IS NOT NULL AND level_data.[POSITION_REPORTS_TO] IS NOT NULL)
    )
    ORDER BY 1;

    -- Debug: Show what the cursor will return for position 40689748
    IF @InputLevel > 1
    BEGIN
        PRINT 'Debug: Cursor results for position 40689748';
        WITH
            PositionData
            AS
            (
        ...
        SELECT DISTINCT
            'Cursor Debug' as DebugStep,
            level_data.[PS_JOB_EMPLID] as Inactive_EMPLID,
            level_data.[POSITION_REPORTS_TO] as Inactive_EMPLID_POSITION_NBR,
            empl2.MANAGER_EMPLID,
            empl2.MANAGER_NAME,
            empl2.POSITION_REPORTS_TO,
            empl2.emplid as empl2_emplid,
            empl2.NAME as empl2_name
        FROM [HealthTime].[stage].[UKG_PositionReportsAnalysis_LEVEL_BY_LEVEL] level_data
            LEFT JOIN health_ods.health_ods.[RPT].[CURRENT_EMPL_DATA] empl2
            ON empl2.POSITION_NBR = level_data.[POSITION_REPORTS_TO]
        WHERE level_data.[POSITION_REPORTS_TO] = '40689748'
            AND level_data.[LEVEL UP] = @InputLevel
            AND level_data.[PS_JOB_EMPLID] IS NOT NULL;
    END
