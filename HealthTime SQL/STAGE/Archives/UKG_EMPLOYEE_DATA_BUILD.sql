...
/****** Object:  StoredProcedure [dbo].[UKG_EMPLOYEE_DATA_BUILD]    Script Date: 7/15/2025 10:28:12 AM ******/
SET ANSI_NULLS ON
GO
...
SELECT   FIN.POSITION_NBR,	MIN(FIN.POSN_SEQ)  AS MIN_POSN_SEQ
  ...
  FROM    HEALTH_ODS.[RPT].[CURRENT_POSITION_PRI_FIN_UNIT]  FIN ,
		dbo.[BUSINESSSTRUCTURE_GET] ()  UKG_BS
...
LEFT OUTER JOIN	  dbo.[BUSINESSSTRUCTURE_GET] ()  UKG_BS
   ON UKG_BS.COMBOCODE =  FIN.FDM_COMBO_CD  
LEFT JOIN dbo.JOBCODES UKG_JC 
   ON UKG_JC.JOBCODE = FIN.JOBCODE
LEFT JOIN dbo.JOBGROUPS UKG_JG 
...
SELECT 
    ...
    WHERE S.NAME IN ( 'STAGE')        
     ...
     AND ( CAST(SUBSTRING (T.NAME,30, 10 ) AS DATE) <  CAST(DATEADD(DAY,-5, GETDATE())   AS DATE)   OR CAST(SUBSTRING (T.NAME,30, 10 ) AS DATE) =  CAST(GETDATE()       AS DATE) )
...
SELECT 
    @SQL_CREATE =  N'SELECT  * INTO [BCK].[' +  @SNAPSHOT_TABLENAME + ']' + ' FROM dbo.UKG_EMPLOYEE_DATA_V';     
