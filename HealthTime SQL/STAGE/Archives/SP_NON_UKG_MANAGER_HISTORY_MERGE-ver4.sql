...
* - Composite key for MERGE/insert logic: [Person Number], [Manager Flag]
* - NOTE column semantics: NULL = active, 'D' = deleted, 'I' = inserted
* - Update_DT: [Update_DT] [date] NULL is populated with GETDATE() when inserting or marking deletions (used instead of snapshot_date for those actions)
...
* - Performance: SourceData CTE reads from [dbo].[UKG_EMPLOYEE_DATA_TEMP]; ensure indexes exist on EMPLID, jobcode, [Person Number]; recommended indexes on target: ([Person Number], [Manager Flag]), [NOTE], [snapshot_date], [hash_value]
*
* Change Summary:
