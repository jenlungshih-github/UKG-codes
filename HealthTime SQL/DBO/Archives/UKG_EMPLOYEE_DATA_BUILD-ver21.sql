...
/****** Object:  StoredProcedure [dbo].[UKG_EMPLOYEE_DATA_BUILD]    Script Date: 12/2/2025 11:36:01 AM ******/
SET ANSI_NULLS ON
GO
...
*                         - Added indexes to NON_UKG_MANAGER_LOG for exclusion lookups
...
FROM STAGE.UKG_EMPL_E_T	  EMPL
    ...
    EXEC [stage].[SP_NON_UKG_MANAGER_LOG_MERGE]
    --12/01/2025 Merges Non-UKG Manager employee (and UKG Manager with BYA) into the NON_UKG_MANAGER_LOG table
    ;
    ...
    -- Update [Custom Field 17] in [stage].[NON_UKG_MANAGER_LOG]
    UPDATE [stage].[NON_UKG_MANAGER_LOG]
SET [Custom Field 17] = B.differncds
FROM [stage].[NON_UKG_MANAGER_LOG] T
    ...
    -- Add index on NON_UKG_MANAGER_LOG for exclusion lookup performance
    DROP INDEX IF EXISTS NON_UKG_MANAGER_LOG_IDX_1 ON [stage].[NON_UKG_MANAGER_LOG];
    DROP INDEX IF EXISTS NON_UKG_MANAGER_LOG_IDX_2 ON [stage].[NON_UKG_MANAGER_LOG];
    CREATE INDEX NON_UKG_MANAGER_LOG_IDX_1 ON [stage].[NON_UKG_MANAGER_LOG] ([Person Number]);
    CREATE INDEX NON_UKG_MANAGER_LOG_IDX_2 ON [stage].[NON_UKG_MANAGER_LOG] (NOTE);
...
EMPLID NOT IN (SELECT emplid
        ...
        where NOTE is NULL)
    --12/01/2025 exclude emplid in NON_UKG_MANAGER_LOG table
    ;
