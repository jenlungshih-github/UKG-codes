...
/****** Object:  StoredProcedure [dbo].[UKG_EMPLOYEE_DATA_BUILD]    Script Date: 9/15/2025 7:29:03 PM ******/
SET ANSI_NULLS ON
GO
...
* -- 09/15/2025 Jim Shih: modify terminated employee filter from "terminated today" to "terminated in past 7 days"
******************************************/
...
FROM STAGE.UKG_EMPL_E_T	  EMPL
    ...
    SELECT distinct
        ...
        ''  AS 'Custom Field 3',
        empl.PAYGROUP AS 'Custom Field 4',
        --CASE WHEN NON_UKG_MANAGER_FLAG = 'T' THEN '' ELSE empl.PAYGROUP  END AS 'Custom Field 4',
        --ISNULL(CAST(UKG_HS.EFFDT AS VARCHAR), '')  AS 'Custom Field 4', -- 9/3/2025  Replace EMPL.EFFDT with UKG_HS.EFFDT  
        --CASE WHEN NON_UKG_MANAGER_FLAG = 'T' THEN '' ELSE empl.EMPL_STATUS END AS 'Custom Field 5',
        ISNULL(EMPL.HR_STATUS, '') AS 'Custom Field 5',
        ''  AS 'Custom Field 6',
        ...
        ''  AS 'Custom Date 3',
        --''  AS 'Custom Date 4',
        ISNULL(CAST(UKG_ES.EFFDT AS VARCHAR), '') AS 'Custom Date 4', -- 9/3/2025  Replace EMPL.EFFDT with UKG_ES.EFFDT  
        ''  AS 'Custom Date 5',
