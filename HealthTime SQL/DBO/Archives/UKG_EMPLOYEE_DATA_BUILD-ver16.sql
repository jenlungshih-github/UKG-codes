...
/****** Object:  StoredProcedure [dbo].[UKG_EMPLOYEE_DATA_BUILD]    Script Date: 12/2/2025 12:17:17 AM ******/
SET ANSI_NULLS ON
GO
...
FROM STAGE.UKG_EMPL_E_T	  EMPL
    ...
    DROP TABLE IF EXISTS   [dbo].[UKG_EMPLOYEE_DATA_TEMP] --------------  Troubleshoot issue from here
...
EXEC [stage].[SP_UKG_EMPL_DATA_TEMP_CleanUp-Step1];

EXEC [stage].[SP_NON_UKG_MANAGER_LOG_MERGE]  --12/01/2025 Merges Non-UKG Manager employee (and UKG Manager with BYA) into the NON_UKG_MANAGER_LOG table
;
...
FROM [stage].[NON_UKG_MANAGER_LOG] T
        INNER JOIN [hts].[UKG_differncds] B -- 12/01/2025 Jim Shih: replace [stage].[UKG_tsr_differncds_V] with [hts].[UKG_differncds]
...
where
EMPLID NOT IN (SELECT emplid FROM STAGE.CTE_exclude_BYA)	--12/01/2025 exclude BYA
AND
EMPLID NOT IN (SELECT [Person Number] from [stage].[NON_UKG_MANAGER_LOG] where NOTE is NULL) --12/01/2025 exclude emplid in NON_UKG_MANAGER_LOG table
;
