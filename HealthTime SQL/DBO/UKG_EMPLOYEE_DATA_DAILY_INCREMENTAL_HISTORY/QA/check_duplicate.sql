-- build reusable temp table of duplicate groups (hash_value + NOTE)
IF OBJECT_ID('tempdb..#duplicate_groups') IS NOT NULL DROP TABLE #duplicate_groups;
...
SELECT hash_value, NOTE, duplicate_count
FROM #duplicate_groups
ORDER BY duplicate_count DESC;

-- Show all rows that share a duplicated (hash_value, NOTE) for detailed inspection
SELECT h.*
FROM dbo.UKG_EMPLOYEE_DATA_DAILY_INCREMENTAL_HISTORY h
    JOIN #duplicate_groups g
    ON h.hash_value = g.hash_value
        AND ISNULL(h.NOTE, '') = ISNULL(g.NOTE, '')
ORDER BY h.hash_value, h.NOTE, h.snapshot_date_TXT, h.EMPLID;
