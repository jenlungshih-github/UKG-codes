...
;
    ...
    INTO [stage].[UKG_EMPL_Inactive_Manager]  -- -- [dbo].[UKG_EMPLOYEE_DATA] empl has reports-to BUT HR_STATUS = 'I'.  Create a table [stage].[UKG_EMPL_Inactive_Manager].
    FROM RankedJobs
    WHERE ROW_NO = 1
...
SELECT emplid,HR_STATUS,JOB_INDICATOR,position_nbr,*
FROM health_ods.health_ods.[RPT].[CURRENT_EMPL_DATA] EMPL
WHERE 
position_nbr='40746409'
;
...
--	and hp.
where 
hp.MANAGER_EMPLID is not NULL
and
L.POSITION_NBR is not NULL
	;
