...
WHERE H.EMPLID = L.EMPLID				
 ...
 SELECT	distinct  
        ...
        ISNULL(CAST(EMPL.EFFDT AS VARCHAR), '')  AS 'Employment Status Effective Date',        
        ...
        CASE WHEN NON_UKG_MANAGER_FLAG = 'T'  THEN '-' 
		     ELSE  ISNULL(UKG_BS.FundGroup, '')  END  AS 'Home Business Structure Level 5 - TSG',
	   ...
	   CASE WHEN NON_UKG_MANAGER_FLAG = 'T' THEN ''
        ...
        CASE WHEN (NON_UKG_MANAGER_FLAG = 'T' OR EMPL.PROB_END_DT IS NULL )THEN '' 
		     WHEN NON_UKG_MANAGER_FLAG = 'F' AND PROBATION_CODE = 'P' THEN CAST(EMPL.PROB_END_DT AS VARCHAR) 
			 ELSE ISNULL(CAST(EMPL.HIRE_DT AS VARCHAR), '') END AS 'Custom Date 1',	       
        CASE WHEN NON_UKG_MANAGER_FLAG = 'T' THEN ''
